      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-gray-400 text-lg max-w-2xl mx-auto">
            Browse equipment and explore exercises with demonstration images
          </p>
        </motion.div>
...
function ExerciseThumbnail({ ex }: { ex: Exercise }) {
  ...
  return (
    <Link
      ...
      className="group relative aspect-[4/3] rounded-lg overflow-hidden bg-gray-900 block"
    >
      <img
        src={imgUrl}
        alt={ex.name}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-2 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
        ...
        <div className="flex items-center gap-1 mt-0.5">
          <span className={cn(
            ...
            ex.difficulty === "Beginner" ? "text-green-300 bg-green-500/20" :
            ex.difficulty === "Intermediate" ? "text-yellow-300 bg-yellow-500/20" :
            "text-red-300 bg-red-500/20"
          )}>
            {ex.difficulty}
          </span>
          {ex.target_muscles[0] && (
            <span className="text-[10px] text-gray-300 truncate">{ex.target_muscles[0]}</span>
          )}
        </div>
      </div>
    </Link>
...
function EquipmentCard({ eq }: { eq: EquipmentWithExercises }) {
  ...
  return (
    <motion.div
      ...
      className="glass rounded-2xl border border-white/5 hover:border-[#DC2626]/20 transition-all duration-300 overflow-hidden flex flex-col"
    >
      <Link href={`/equipment/${eq.slug}`} className="p-5 block hover:bg-white/[0.02] transition-colors">
      ...
      {hasExercises && (
        <div className="border-t border-white/5 px-5 py-4">
          <div className="flex items-center justify-between mb-3">
            <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Exercises</h4>
            <Link
              href="/exercises"
              className="inline-flex items-center gap-1 text-xs text-[#DC2626] hover:text-white transition-colors"
            >
